...
- For Snowflake, verify that the service user's default role, schema privileges, key pair, row limits, and replica identity settings match the [Snowflake destination guide](/docs/guides/database/replication/snowflake).
...
When a table encounters an error during the initial sync, you can reset the table state. This deletes the table's existing destination data and restarts its initial sync from the beginning. For Snowflake, the reset drops and recreates the append-only history table and its managed streaming channel.
